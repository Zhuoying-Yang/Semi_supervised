...
# --- THE "SAVE YOURSELF" BYPASS ---
if "data_preparation" not in sys.modules:
    m = ModuleType("data_preparation")
    sys.modules["data_preparation"] = m

class SleepDataset_2_chan(Dataset):
    def __len__(self):
        return len(self.label)

    def __getitem__(self, idx):
        pid = self.patient_no[idx] if (hasattr(self.patient_no, '__len__') and not isinstance(self.patient_no, str)) else self.patient_no
        return self.ch1[idx], self.ch2[idx], self.label[idx], pid

sys.modules["data_preparation"].SleepDataset_2_chan = SleepDataset_2_chan
# ----------------------------------
...
def get_args():
    ...
    parser.add_argument("--save_dir", type=str, default="/scratch/zhuoying/sleep_results_ssl")
    parser.add_argument("--pretrained_path", type=str, default="/scratch/zhuoying/sleep_results/sleepedf_pretrained_weights.pth")
...
class CNNOnlySleepNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # --- THE FIX: ON-THE-FLY RESAMPLING ---
        if time != 3000:
            x = F.interpolate(x, size=3000, mode='linear', align_corners=False)
        # --------------------------------------
...
def run_fold(target_fold, args, device):
    print(f"\n{'='*40}\nSTARTING FOLD: {target_fold} (SEMI-SUPERVISED TRANSFER)\n{'='*40}")
    set_seed(args.seed)
    ...
    if os.path.exists(args.pretrained_path):
        print(f"Loading pre-trained Sleep-EDF weights from {args.pretrained_path}...")
        model.load_state_dict(torch.load(args.pretrained_path))
    else:
        print(f"WARNING: Pre-trained weights not found. Training from scratch.")

    print("Backbone PERMANENTLY frozen to protect pre-trained features. Training temporal head with SSL...")
    for param in model.cnn_backbone.parameters():
        param.requires_grad = False

    # Optimizer ONLY updates the temporal head and fully connected layer
    optimizer = optim.AdamW(filter(lambda p: p.requires_grad, model.parameters()), lr=args.lr, weight_decay=3e-2)
    scheduler = optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=args.epochs, T_mult=1)

    best_val_acc = 0.0
    best_model_path = os.path.join(args.save_dir, f"best_ssl_model_fold_{target_fold}.pth")
