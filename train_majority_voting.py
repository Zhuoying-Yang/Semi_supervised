...
class SeqDataset(Dataset):
    ...
    def __getitem__(self, idx):
        # Now returns data, labels, and the unique global indices of each segment in the sequence
...
def create_sequences(data_list, seq_len=21):
    sequences = []
    # Sort by PID and then by time/index to ensure temporal order
    data_list.sort(key=lambda x: int(x[3])) 
    
    # We assign a unique ID to every segment based on its position in the original list
    # item[3] is PID, item[4] (if exists) would be original time index. 
    # Here we use the list index as a global segment ID.
    indexed_data = []
    for i, item in enumerate(data_list):
        # item structure: (ch1, ch2, label, pid) -> we add global_idx
    ...
    for item in indexed_data:
        ...
        cur_y.append(item[2])
        cur_idx.append(item[4]) # This is the global segment index
        last_pid = pid
...
def run_fold(target_fold, args, device):
    print(f"\n--- PROCESSING FOLD: {target_fold} (CNN-ONLY WITH VOTING) ---")
    set_seed(args.seed)
    ...
    val_data_list = [val_obj[i] for i in range(len(val_obj))]
    val_loader = DataLoader(SeqDataset(create_sequences(val_data_list, args.seq_len)), batch_size=32)
    ...
    for epoch in range(args.epochs):
        ...
        for x_l, y_l, _ in l_loader: # Added _ for indices (unused in training)
            ...
            if random.random() > 0.5:
                ...
                loss_sup = lam * F.cross_entropy(logits, ya.view(-1)) + \
                           (1-lam) * F.cross_entropy(logits, yb.view(-1))
            else:
        ...
        # --- VALIDATION WITH PROBABILISTIC SUMMING (VOTING) ---
        model.eval()
        # segment_agg: Global_Index -> Summed Softmax Probabilities
        # segment_gt: Global_Index -> Ground Truth Label
        ...
        # Perform the final "Vote" for each unique segment
        correct = 0
        total_segments = len(segment_gt)
        for g_idx, total_prob in segment_agg.items():
            final_pred = torch.argmax(total_prob).item()
            if final_pred == segment_gt[g_idx]:
                correct += 1
        
        acc = correct / total_segments if total_segments > 0 else 0
        print(f"Fold {target_fold} | Epoch {epoch+1} | Voted Acc: {acc:.4f} | SSL: {current_ramp_weight:.3f}")
