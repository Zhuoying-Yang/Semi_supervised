...
def get_args():
    ...
    parser.add_argument("--save_dir", type=str, default="/scratch/zhuoying/sleep_results/baseline_100")
...
# --- MODEL ARCHITECTURE --- (Identical to your SSL version)
...
def run_fold(target_fold, args, device):
    print(f"\n--- FULL DATA BASELINE (100% SUPERVISED) FOLD: {target_fold} ---")
    set_seed(args.seed)
    ...
    # Use 100% of the training split
    train_seqs = create_sequences([x for x in train_list if int(x[3]) in train_pids], args.seq_len)
    val_seqs = create_sequences([x for x in train_list if int(x[3]) in val_pids], args.seq_len)
    
    test_obj = torch.load(os.path.join(args.data_dir, str(target_fold), "val_set.pt"), weights_only=False)
    test_seqs = create_sequences([test_obj[i] for i in range(len(test_obj))], args.seq_len)
    del train_obj, train_list, test_obj; gc.collect()

    train_loader = DataLoader(SeqDataset(train_seqs), batch_size=16, shuffle=True)
    ...
    best_val_acc, best_path = 0.0, os.path.join(args.save_dir, f"best_100_fold_{target_fold}.pth")
    ...
    print(f"\n--- FINAL 100% BASELINE TEST FOLD {target_fold} ---")
